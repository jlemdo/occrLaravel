 ...
 <nav class="navbar navbar-top fixed-top navbar-expand" id="navbarDefault">
        <div class="collapse navbar-collapse justify-content-between">
          <div class="navbar-logo">
			...
			<!--Holio <span class="text-1000 d-none d-sm-inline">GtT</span>-->
			 <div class="d-flex align-items-center"><img src="{{asset('assets/logo.png')}}" alt="Lácteos y más" width="50" />
                </div>
			</a>
          </div>
      ...
      <label class="mb-0 theme-control-toggle-label theme-control-toggle-light" for="themeControlToggle" data-bs-toggle="tooltip" data-bs-placement="left" title="Cambiar tema"><span class="icon me-1 d-none d-sm-block" data-feather="moon"></span><span class="fs--1 fw-bold">Oscuro</span></label>
      <label class="mb-0 theme-control-toggle-label theme-control-toggle-dark" for="themeControlToggle" data-bs-toggle="tooltip" data-bs-placement="left" title="Cambiar tema"><span class="icon me-1 d-none d-sm-block" data-feather="sun"></span><span class="fs--1 fw-bold">Claro</span></label>
              </div>
            </li>
    ...
    <a href="{{ $notification->data['url'] }}" class="btn btn-link">Ver Detalles</a>
@endif</p>
    <p class="text-800 fs--1 mb-0"><span class="me-1 fas fa-clock"></span>{{$notification->created_at}}</p>
                            ...
                            <div class="font-sans-serif d-none d-sm-block">
                              ...
                              @if($notification->read_at !=NULL)
              <a class="dropdown-item" href="{{ route('notifications.markAsUNRead', $notification->id) }}">Marcar como No Leída</a>
              @else 
              <a class="dropdown-item" href="{{ route('notifications.markAsRead', $notification->id) }}">Marcar como Leída</a>
              @endif
                               </div>
                            </div>
                          </div>
                  ...
                  <div class="card-footer p-0 border-top border-0">
                    ...
                    <a class="fw-bolder" href="{{route('notifications')}}">Todas las Notificaciones</a></div>
                  </div>
                </div>
              </div>
            </li>
                ...
                <div class="card position-relative border-0">
                  <div class="card-body p-0">
                    <div class="text-center pt-4 pb-1">
					  ...
					  <h6 class="mt-2 text-black">Sesión como {{auth()->user()->usertype == 'admin' ? 'Administrador' : (auth()->user()->usertype == 'driver' ? 'Conductor' : auth()->user()->usertype)}}</h6>
                      <hr />
                    </div>
                  </div>
    <div class="overflow-auto scrollbar" style="height: 7rem;">
      <ul class="nav d-flex flex-column mb-1 pb-1">
        <li class="nav-item"><a class="nav-link px-3" href="{{route('dashboard')}}"><span class="me-2 text-900" data-feather="settings"></span>Panel</a></li>
        <li class="nav-item"><a class="nav-link px-3" href="{{route('users.editprofile')}}"> <span class="me-2 text-900" data-feather="user"></span><span>Perfil</span></a></li>
        <li class="nav-item"><a class="nav-link px-3" href="{{route('profile.edit',auth()->user()->id)}}"> <span class="me-2 text-900" data-feather="lock"></span>Editar Cuenta</a></li>
...
<a class="btn btn-phoenix-secondary d-flex flex-center w-100" class="dropdown-item" href="javascript:void(0);"
  onclick="event.preventDefault(); this.closest('form').submit();"><span class="me-2" data-feather="log-out"> </span>Cerrar Sesión</a>
</form>
